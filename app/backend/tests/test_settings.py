...
def test_settings_loads_from_env(monkeypatch) -> None:
    monkeypatch.setenv("REDLINE_ANTHROPIC_API_KEY", "key-xyz")
    monkeypatch.setenv("REDLINE_AGENT_TIMEOUT_S", "12.5")
...
def test_redacting_filter_masks_known_keys(caplog) -> None:
    ...
    logger = logging.getLogger("redline.test_redact")
    with caplog.at_level(logging.INFO, logger="redline.test_redact"):
