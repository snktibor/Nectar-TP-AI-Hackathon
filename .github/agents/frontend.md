...
## Scope
Implement and maintain the ingest-first React/Vite dashboard with strict document intake rules, backend-driven audit reporting, and client-side enterprise PDF export.

## Core Screens
- Split workspace with left upload panel and right audit/report workspace.
- Dashboard shell with left navigation rail, local click state only, no profile footer block, and full-height mobile drawer navigation with locked background scroll.
...
- Ingest progress and classification result cards for each processed file; processed state hides the upload instruction block and format badge.
- Re-upload flow from results view via `Újrakezdés` beside the category status, resetting upload state and reopening the picker.
...
- PDF evidence viewer with citation-page conversion, target-page-first loading, and best-effort quote highlighting.
- Completed report tabs: findings, agent runs, telemetry.
- Finding cards show severity plus a finding-group badge (`Konzisztencia hibák`, `Benchmark kockázatok`, `Hiányzó elemek`).
- Report generator modal creates the enterprise PDF in the browser with `@react-pdf/renderer`, centered `PDF letöltés` CTA, and confidentiality notice.
...
- Use `src/lib/citations.ts` for backend evidence page label formatting and conversion before opening `DocumentViewer`.
- Use `src/lib/findingFilters.ts` for selected-document finding panels and document finding count badges.
...
- Do not pass backend `EvidenceChunk.page` directly to `DocumentViewer`; `CitationTarget.page` is a zero-based PDF viewer index.
- Document-scoped finding views must match explicit filenames, locations, and evidence chunks before falling back to `doc_type_scope`.
